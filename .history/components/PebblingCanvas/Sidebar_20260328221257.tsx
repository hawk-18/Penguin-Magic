...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  // 轮询视频任务状态
  const pollVideoTask = async (taskId: string, maxAttempts: number = 120, intervalMs: number = 5000) => {
  ...
  const handleComposerSubmit = async () => {
    ...
    const isVideo = kind === 'native-video';
    ...
    if (isVideo) {
      ...
      try {
        ...
        // 如果有参考图片
        if (floatingComposer?.sourceImageUrl) {
          body.referenceMediaUrls = [floatingComposer.sourceImageUrl];
        }
        ...
        // 同时触发画布节点创建
        window.dispatchEvent(new CustomEvent('pebbling-composer-preview', {
          detail: { url: videoUrl, isVideo: true },
        }));
                ...
                <div className="px-4 pt-3 pb-3">
                  <div className="relative w-full overflow-hidden rounded-xl border border-white/10 bg-zinc-800/90 min-h-[200px] flex flex-col">
                    ...
                    <div className="flex items-center gap-1.5 px-3 pt-2 pb-1 text-[11px] text-zinc-400">
                      {floatingComposer?.kind === 'native-video' ? (
                        <>
                          <Icons.Video size={12} className="opacity-70" />
                          <span>视频生成</span>
                        </>
                      ) : (
                        <>
                    ...
                    <div className="relative flex-1 min-h-[140px] flex items-center justify-center mx-2 mb-2 rounded-lg bg-zinc-900/80 border border-white/5">
                      {/* 视频生成进度显示 */}
                      {floatingComposer?.kind === 'native-video' && videoTaskStatus && videoTaskStatus !== 'completed' && (
                        <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-black/60 backdrop-blur-[2px]">
                          ...
                          <div className="text-[12px] text-white font-medium mb-1">
                            {videoTaskStatus === 'pending' && '任务排队中...'}
                            {videoTaskStatus === 'running' && '视频生成中...'}
                      ...
                      {/* 图片生成loading */}
                      {composerGenerating && floatingComposer?.kind !== 'native-video' && (
                      ...
                      {composerPreviewMedia?.url ? (
                        ...
                        <div className="flex flex-col items-center justify-center gap-2 p-4 w-full">
                          ...
                          <span className="text-[11px] text-zinc-500">
                            {floatingComposer?.kind === 'native-video' ? '视频结果将显示在此处' : '生成结果将显示在此处'}
                          </span>
                        </div>
                      ) : (
                        <div className="flex flex-col items-center justify-center gap-2 text-zinc-500">
                          {floatingComposer?.kind === 'native-video' ? (
                            <Icons.Video size={40} className="opacity-25" />
                          ) : (

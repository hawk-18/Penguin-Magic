...
/** 浮动面板左上角屏幕坐标 → 锚点节点在画布上的位置（与 Sidebar 预览区高度一致） */
...
const PebblingCanvas: React.FC<PebblingCanvasProps> = ({ 
  ...
  useEffect(() => {
    const onOpenComposer = (e: Event) => {
      ...
      const nextX = Math.max(80, Math.min(window.innerWidth - 820, px + 16));
      ...
      setNodes((prev) => [...prev, anchorNode]);
      setConnections((prev) => [...prev, { id: uuid(), fromNode: sourceNodeId, toNode: anchorId }]);
    ...
    const onDismiss = (e: Event) => {
      ...
      if (!aid) return;
      setNodes((prev) => prev.filter((n) => n.id !== aid));
      setConnections((prev) => prev.filter((c) => c.fromNode !== aid && c.toNode !== aid));
      setHasUnsavedChanges(true);
    };
  ...
  const handleStartConnection = (nodeId: string, portType: 'in' | 'out', pos: Vec2) => {
    ...
    const cr = container.getBoundingClientRect();
    const currPos = {
      x: (pos.x - cr.left - canvasOffset.x) / scale,
      y: (pos.y - cr.top - canvasOffset.y) / scale,
    };
    setLinkingState({
      active: true,
      fromNode: nodeId,
  ...
  const handleGenerate = async (type: NodeType, prompt: string, config: GenerationConfig, files?: File[]) => {
      ...
      try {
          if (type === 'image') {
               ...
               if (!result) {
                 alert(
                   '未生成出图片。请确认已在「设置」中启用贞贞 API 并填写 Key，或在欢迎页配置 Gemini API Key；详情见浏览器控制台 (F12)。'
                 );
               }
               // 同步到桌面
               if (result && onImageGenerated) {
               ...
               if (!result) {
                 alert(
                   '编辑未返回图片。请确认 API 已配置且网络正常；详情见浏览器控制台 (F12)。'
                 );
               }
               // 同步到桌面
               if (result && onImageGenerated) {
  ...
  useEffect(() => {
    const onComposerSubmit = async (e: Event) => {
      ...
      const sourceNodeId = String(detail.sourceNodeId || '');
      const anchorId = String(detail.anchorId || '');
      let refImage: string | undefined;
      if (sourceNodeId) {
        const sn = nodesRef.current.find((n) => n.id === sourceNodeId);
        if (sn?.content && isValidImage(sn.content)) {
          refImage = sn.content;
        }
      }
      if (kind === 'remove-bg' && refImage) {
        finalPrompt = `${finalPrompt}\n【任务】保留画面主体，替换或去除原背景，生成自然融合的新背景。`;
      }
      if (kind === 'expand-image' && refImage) {
        finalPrompt = `${finalPrompt}\n【任务】在保持主体不变的前提下扩展画布、补全画面边缘，风格与光影一致。`;
      }
      ...
      if (task === 'image') {
        ...
        if (refImage) {
          body.referenceMediaUrls = [refImage];
        }
      }
      ...
      try {
        for (let i = 0; i < generationCount; i++) {
          ...
          // 豆包 / ComfyUI 图片：期望返回 url（ComfyUI 由后端轮询 /history 后统一为 data[0].url）
          if (task === 'image') {
            const url =
              json.data?.data?.[0]?.url ||
              json.data?.data?.[0]?.b64_json ||
              json.data?.data?.url ||
              json.data?.url;
            const nodeType: NodeType = kind === 'expand-image' ? 'edit' : 'image';
            if (typeof url === 'string' && url) {
              let place: Vec2 | undefined;
              if (anchorId) {
                const an = nodesRef.current.find((n) => n.id === anchorId);
                if (an) {
                  place = { x: an.x + 48, y: Math.max(0, an.y - 40) };
                }
              }
              addNode(nodeType, url, place);
              window.dispatchEvent(
                new CustomEvent('pebbling-composer-preview', {
                  detail: { url, isVideo: false },
                })
              );
            } else {
              alert(
            ...
            if (taskId) {
              ...
              addNode('video', String(taskId), place);
            }
          }
        }
      } finally {
  ...
  return (
    <div 
      className={`w-full h-full overflow-hidden relative ${themeName !== 'light' ? 'text-white' : 'text-black'}`}
        ...
        >
            ...
            <svg className="absolute top-0 left-0 w-full h-full overflow-visible pointer-events-none z-0">
                ...
                {connections.map(conn => {
                    // 🔧 使用 nodesRef + 浮动面板虚拟节点，确保拖拽时连线实时跟随
